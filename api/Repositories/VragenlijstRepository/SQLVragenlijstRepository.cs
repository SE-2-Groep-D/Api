...
public class SQLVragenlijstRepository : IVragenlijstRepository {
  ...
  public async Task<Questionlist> UpdateAsync(Guid id, Questionlist updatedQuestionlist) {
    var existingQuestionlist = await _context.Questionlist
      .Include(q => q.Questions)
      .ThenInclude(q => q.PossibleAnswers)
      .FirstOrDefaultAsync(q => q.Id == id);

    if (existingQuestionlist == null) {
      throw new Exception("Questionlist not found");
    }

    // Update de hoofdvelden
    existingQuestionlist.Title = updatedQuestionlist.Title;
    existingQuestionlist.Description = updatedQuestionlist.Description;

    // Behandel de vragen
    foreach (var updatedQuestion in updatedQuestionlist.Questions) {
      var question = existingQuestionlist.Questions
        .FirstOrDefault(q => q.Id == updatedQuestion.Id);

      if (question == null) {
        // Nieuwe vraag
        existingQuestionlist.Questions.Add(updatedQuestion);
      }
      else {
        // Update bestaande vraag
        question.Title = updatedQuestion.Title;
        question.Type = updatedQuestion.Type;
        // Update mogelijke antwoorden indien nodig
      }
    }
